...
class Detector(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, faces_data):
        ...
        # Create DataFrames
        feat_poses = pd.DataFrame(poses_np, columns=FEAT_FACEPOSE_COLUMNS_6D)
        feat_aus = pd.DataFrame(aus, columns=AU_LANDMARK_MAP["Feat"])

        # If poses data is empty (contains NaN), make corresponding AU data empty too
        if has_empty_poses.any():
            for i, is_empty in enumerate(has_empty_poses):
                if is_empty:
                    # Set all AU values to NaN for this row
                    feat_aus.iloc[i, :] = np.nan

        feat_data = pd.concat([feat_poses, feat_aus], axis=1)
